...
const root=process.argv[2],out=path.join(__dirname,'held-thunder-spire');fs.mkdirSync(out,{recursive:true});
const rows=fs.readFileSync(path.join(root,'sprite-manifest.csv'),'utf8').trim().split(/\r?\n/),keys=rows.shift().split(',');
const entries=rows.map(r=>Object.fromEntries(r.split(',').map((v,i)=>[keys[i],v]))).filter(e=>e.subspace==='equipment'&&e.entry==='staff').sort((a,b)=>+a.frame-+b.frame);
...
execFileSync('ffmpeg',['-v','error','-n','-f','rawvideo','-pix_fmt','rgba','-s','252x612','-i','-','-frames:v','1',path.join(out,'standard-staff-reference.png')],{input:sheet});
