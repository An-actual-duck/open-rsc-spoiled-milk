...
| Plugins and extensions | `server/src/.../plugins/`, `server/plugins/` | `server/src/com/openrsc/server/extensions/`, configuration/profile selection, plugin tests | Discovery, trigger registration, defaults, and reflection make text references incomplete. Core and plugin artifacts must be compiled together. |
