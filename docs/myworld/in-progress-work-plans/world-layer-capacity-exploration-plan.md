...
owner-validated Slice 125 exposes bounded composed-target results and every
outer refusal through additive private schema-v43, corrects both runtime source
paths, and proves one pending authored stump passes the stable lifecycle,
Region-boundary, and dormant contract checks before natural completion clears
the record, while preserving v42 and all non-authoritative boundaries;
automated-validated Slice 126 derives only a dormant exact mutation intent from
...
automated-validated Slice 130 adds the disconnected real per-Region monitor and
non-creating canonical RegionManager runner, mechanically proving same-Region
exclusion and cross-Region ordered acquisition while leaving every existing
object, collision, event, and callback mutation path unchanged;
automated-validated Slice 131 adds a pure exact collision-footprint planner for
...
replacement and scheduler-store integration remain refused; and
automated-validated Slice 139 connects the exact zero-attribute authored-
transient snapshot and reversible collision contribution to that disconnected
consumer, while opaque or asymmetric rollback state refuses and the scheduler
Store remains disconnected; and
...
RegionManager remains disconnected; and
...
automated-validated Slice 159 classifies every proposal-related callback before
live recovery work, separates total/complete/incomplete and attribution counts,
...
refusal boundary. Its corrected private capture proves all 449 selected
...
automated-validated Slice 166 extends each exact owner fence to supporting
callbacks outside the proposal-related subset and adds one per-NPC preservation
gate shared by movement, behavior, removal, respawn, and World-removal state.
...
boundary;
automated-validated Slice 169 copies the inventory's exact canonical packed
source order into the owner requirements and admits a caller only through a
...
owner-validated Slice 172 exposes that safe refusal only through the
...
automated-validated Slice 174 inventories exact Region-local source-absence
blockers inside that boundary. It separates active players, NPC membership,
authored and dynamic objects, ground items, collision products, tile storage,
...
- this was a diagnostic wiring omission, not a world-event or visual failure.
  ...
  interaction in both routes; and
- after the command-source correction, all seven final records validate against
  the local v1-v43 schema registry. `atomic-pending` retains registration `3961`
  at observation tick `2756`, lifecycle version `48 -> 48`, with one exact
  authored transient classified under the Region object boundary and
  `MUTATION_PRECONDITION_REVALIDATED`; `atomic-after` at tick `2824` and the
  stop record contain zero restoration records; broad human timing was
  sufficient throughout.

Status: implemented, automated-validated, and owner-validated. No mutation or
lifecycle authority is authorized.

### Slice 126: Dormant exact mutation intent

Objective: define the smallest exact authored scenery mutation description
that can exist only after stable satisfied Slice 118 evidence, without turning
the detached result into mutation authority or connecting it to runtime.

Implemented:

- the target-revalidation request now retains `forceFullBlock`, the final known
  scenery-spawn constructor scalar that an exact future intent would require;
  removal requests reject a true value, and authored source ordinals retain the
  existing global bound;
- `GameTickEventRestorationMutationIntent.assess` applies one ordered refusal
  table to positive equal lifecycle versions, Region availability, real object-
  boundary classification, the satisfied mutation-precondition contract,
  exact target-count/state agreement, operation-specific outcome/reason, and
  construction-kind/type agreement;
- only accepted spawn-empty, spawn-exact-authored-transient, and removal-exact-
  restoration-target states construct an intent. No-op, refused, unavailable,
  lifecycle-changing, count-inconsistent, operation-mismatched, and unsupported
  construction states retain explicit refusal reasons and no intent;
- the accepted immutable value copies operation, desired present/absent state,
  expected target state, scenery constructor fields, force-full-block, and
  authored generation/source identity; and
- scheduler instance, registration sequence, lifecycle version, event,
  callback, Region, entity, collection, owner, runtime attributes, key, UUID,
  and every runtime handle remain absent. Store, handler, RegionManager,
  observer, loading, retirement, arrival, and event execution do not consume
  the value.

Automated validation status:

- an executable Java fixture accepts exact empty/transient spawn intents and an
  exact removal intent, retaining force-full-block and desired state;
- the same fixture exercises lifecycle, Region, no-op, refused target,
  contradictory count, operation mismatch, construction mismatch, unsupported
  construction, removal force-block, and authored-ordinal refusal paths;
- source guards prove the value has no runtime/mutation capability, remains
  disconnected from all three plausible runtime consumers, and that the
  request/store path does not drop force-full-block;
- the complete layered-map suite passes 406 tests across 125 focused files;
  and
- the authoritative bundled-Ant server build compiles 781 core and 488 plugin
  sources and passes its build/classpath audit.

Safety boundary:

- the intent is stale immediately after its source Region boundary releases;
  it is not a reusable permit, authorization, commit token, achieved-state
  claim, or proof that its target still matches;
- constructing it performs no lookup, comparison, mutation, callback,
  cancellation, reschedule, packet, persistence, loading, retirement,
  reconstruction, rollback, or arrival operation; and
- no runtime path can currently obtain or consume it. A later compare-and-apply
  design must re-establish every boundary and compare the exact target again.
...
- source guards prove the contract imports no runtime model and performs no
  lock, tile, object, callback, or packet operation. Slice 130 later consumes
...
- no Slice 126-129 value reaches runtime registration, callback execution,
  ...
  Slice 130's coordinate-only boundary seam does not alter that guarantee.

Status: implemented and automated-validated. No owner route is required for
this disconnected transaction specification; no lock, rollback, mutation, or
lifecycle authority is authorized.

### Slice 130: Disconnected ordered Region boundaries

Objective: establish and mechanically prove the smallest real canonical
multi-Region monitor foundation while keeping it disconnected from all object,
collision, event, and callback mutation paths.

Implemented:

- every `Region` owns one package-local
  `RegionObjectCollisionMutationBoundary` keyed by its immutable packed Region
  coordinate; the monitor is distinct from the existing object collection and
  tile-expansion monitors so no historical lock order is silently changed;
- `executeReadOnly` accepts a bounded nonempty list only in strict
  `(regionX, regionY)` order, rejects reverse order and duplicates before
  acquiring a monitor, nests monitor acquisition recursively, invokes one
  void read-only operation only while every monitor is held, and releases in
  reverse order through monitor unwinding;
- the held-boundary view copies only immutable coordinates and proves with
  `Thread.holdsLock` that the full set is held during the operation. Neither the
  operation nor a result value is retained;
- RegionManager's package-local helper validates the same canonical coordinate
  order, holds the existing Region lifecycle boundary, resolves only through
  non-creating `peekRegionFromSectorCoordinates`, returns a typed unavailable
  refusal if any Region is absent, and otherwise delegates to the ordered
  runner; and
- no World registration, unregistration, replacement, tile update, target
  observation, Store fence, handler, callback, packet, or public API consumes
  the new boundary.

Automated validation status:

- a deterministic two-thread latch fixture proves same-Region exclusion: the
  second operation cannot enter until the first releases its monitor;
...
- source guards prove Region creates exactly its boundary, RegionManager uses
  ...
  TileValue, callback, or packet type;
- the complete layered-map suite passes 422 tests across 129 focused files;
  and
- the authoritative bundled-Ant server build compiles 785 core and 488 plugin
  sources and passes its build/classpath audit.

Safety boundary:

- “read-only” is an API contract, not a Java capability sandbox; the runner is
  package-local and deliberately has no runtime caller while later specialized
  operations are designed;
- ordinary object and collision mutations do not yet share these monitors, so
  holding them does not currently prove stable world state or satisfy Slice
  129's shared-mutation prerequisite; and
- completed execution is not a permit, mutation/rollback authority, atomicity
  claim, commit token, arrival gate, or lifecycle authority.

Status: implemented and automated-validated. No owner route is required for
this disconnected lock-order foundation; no world mutation behavior changed.

### Slice 131: Exact collision-footprint planner

Objective: reproduce the legacy collision contribution for one detached
scenery or boundary constructor so future mutation code can know the complete
ordered Region set before acquiring Slice 130's monitors.
...
- scenery width/height rotation follows directions `0/4` versus all other
  ...
  bits and their adjacent opposing bits;
- boundary door type 1 preserves its separate direction `0-3` collision table,
  including the legacy direction-0 mismatch between the west projectile
  neighbor and south collision neighbor;
- projectile contributions are counted rather than collapsed, so overlap from
  adjacent footprint cells remains exact, while six-bit dynamic collision
  contributions are combined per unique tile;
- the delayed force-full-block addition is accumulated independently on the
  anchor, including after object ID 1147's register-only early return;
- the planner preserves the legacy object ID 1147 asymmetry: register produces
  no ordinary collision and requires no definition, while unregister still
  follows its resolved definition;
- every successful result contains immutable row-major unique contribution
  tiles plus an immutable strict `(regionX, regionY)` Region set that always
  includes the anchor Region, even for a collisionless object; and
- missing ordinary definitions, constructor/definition-kind mismatch,
  oversized footprints, overflow, any out-of-world anchor/footprint/neighbor
  effect, and force-full-block on unregister refuse without returning a partial
  footprint.

Automated validation status:

- executable table fixtures prove rotated two-tile blocking scenery and an
  additional force-full-block contribution across a packed Region boundary;
- a rotated directional fixture proves opposing wall-bit aggregation and a
  projectile count of two on an overlapping tile;
- a boundary fixture proves the distinct legacy projectile and collision axes;
- object ID 1147 fixtures prove its collisionless register, forced anchor
  addition, and definition-driven unregister behavior;
- missing/mismatched definitions, out-of-world neighbors, oversized
  footprints, invalid force usage, empty collision footprints, defensive
  copies, canonical Region order, and all false authority flags are checked;
- source guards prove World, RegionManager, Store, and handler do not consume
  the planner, and the planner performs no runtime observation, locking, tile
  access, or mutation;
- the complete layered-map suite passes 426 tests across 130 focused files;
  and
- the authoritative bundled-Ant server build compiles 786 core and 488 plugin
  sources and passes its build/classpath audit.
...
- the result does not authorize registration, unregistration, collision apply,
  rollback, packets, arrival, retirement, or lifecycle change.
...
### Slice 134: Disconnected ordered collision executor

Objective: mechanically prove the first real TileValue apply seam under Slice
130's canonical Region monitors while keeping all ordinary World object and
callback paths disconnected.

Implemented:

- `RegionObjectCollisionMutationBoundary` retains its inert read-only entry
  point and adds a distinct package-local mutation operation that acquires the
  same strict canonical monitor set and invokes work only while every monitor
  is held;
- `RegionCollisionFootprintMutationExecutor` requires boundary coordinates to
  exactly equal Slice 131's anchor-plus-contribution Region coverage before
  tile access;
- inside those monitors it captures exact current blocking, six-counter
  dynamic-collision, and projectile state for every contribution tile, invokes
  Slice 133 with a fresh in-boundary declaration, and refuses before mutation
  if any tile or arithmetic precondition is unavailable;
- an accepted projection applies only the exact counted contribution through
  existing TileValue APIs, verifies every projected counter before monitor
  release, and reverses the contribution in reverse tile order if post-state
  verification ever fails;
- RegionManager exposes one package-local lifecycle-protected adapter that
  resolves only existing Regions, maps footprint tiles to their already-
  resolved Region, and delegates to the executor; and
- World registration, unregistration, replacement, delayed callbacks, object
  membership, packets, visibility invalidation, and persistence do not call the
  new executor.

Automated validation status:

- a deterministic multi-Region fixture proves exact register/unregister
  add/remove round-trip across packed Region X `0 -> 1`;
- a two-thread latch proves same-footprint mutations cannot enter concurrently
  and that the second enters after canonical monitor release;
- underflow refusal and missing-tile paths complete without partially changing any
  counter, while reversed boundary coverage refuses before tile access;
- applied results report exact boundary count and mutation completion, refused
  results report no mutation authority, and rollback/commit/arrival/lifecycle
  flags remain closed except the executor's internal verified reversal path;
- source guards prove only RegionManager references the package-local executor
  and that no World, Store, handler, object-membership, callback, or packet path
  consumes it;
- the complete layered-map suite passes 438 tests across 133 focused files;
  and
...
- the executor is real mutation capability but has no ordinary runtime caller;
  its deterministic fixture owns isolated TileValues;
- existing World collision mutations still do not share Slice 130's monitors,
  so holding them outside this executor does not yet prove stable live state;
  and
- object collection membership is not part of this executor. It cannot make
  registration/unregistration atomic, restore an object, emit packets, gate
  arrival, or alter Region lifecycle.
...
### Slice 139: Exact authored-transient replacement

Objective: connect Slice 128's detached exact transient snapshot to Slice 138's
ordered Region consumer so one rollback-complete authored stump/depleted-object
replacement can commit atomically without weakening the refusal boundary for
opaque or asymmetric state.

Implemented:

- the transient snapshot accepts an ephemeral Slice 137 commit request and
  revalidates its target coordinate, object type/direction, authored generation,
  source coordinate, ordinal, and construction kind against detached candidate
  scalars;
- the object/collision executor constructs that candidate only after the fresh
  Region target classifier reports one exact authored transient and while both
  the canonical Region union and anchor GameObject transaction monitor are
  held;
- the capture includes the transient constructor, owner, runtime-attribute
  count, exact authored identity, exact slot count, and the candidate's bounded
  unregister collision contribution;
- unregister and rollback-register footprints must have identical canonical
  Regions and every per-tile blocking, directional, and projectile count;
  legacy saturating unregister is explicitly non-reversible;
- one exact zero-attribute, reversibly projected transient is removed and the
  desired authored object is registered through the existing atomic membership/
  collision transaction; and
- opaque runtime attributes refuse snapshot creation, while object ID 1147's
  asymmetric collision behavior refuses before membership or collision changes.

Automated validation status:

- the executable fixture proves exact authored-transient replacement removes
  the old membership, installs the desired membership, and leaves the correct
  blocking collision contribution;
- the same fixture proves opaque runtime attributes refuse unchanged and the
  collisionless-register/saturating-unregister asymmetry refuses unchanged;
- source guards require snapshot construction inside the transient transaction
  path, exact collision-reversal comparison, no runtime handles in the detached
  snapshot, and that the scheduler Store remains disconnected;
- the complete layered-map suite passes 458 tests across 138 focused files;
  and
- the authoritative bundled-Ant server build compiles 791 core and 488 plugin
  sources and passes its build/classpath audit.
...
- snapshot validation is now connected to the disconnected Region consumer,
  so it is no longer described as dormant; the value itself still performs no
  runtime observation or mutation and retains no runtime handle;
- a transient with opaque runtime attributes, incomplete identity, changed
  slot membership, non-reversible collision projection, unavailable Region or
  tile state, or collision arithmetic refusal cannot be replaced;
- the scheduler Store remains disconnected. No current callback or gameplay
  path reaches this restoration seam, and no callback is invoked, consumed,
  cancelled, or rescheduled; and
- event consumption, arrival ordering, Region retirement, reconstruction,
  loading, persistence, and true layered storage remain later gates.

Status: implemented and automated-validated. No owner route is required because
the scheduler Store remains disconnected and gameplay cannot reach this seam.

### Slice 140: One-shot restoration consumption contract

Objective: specify the exact scheduler-local disposition of the pending
one-shot callback after a future Region restoration attempt, including closed
postconditions for both refusal and desired-state satisfaction before any
runtime consumer is connected.

Implemented:

- `GameTickEventRestorationOneShotConsumptionContract` accepts only detached
  scheduler identity, registration, generation, boundary, event timing,
  execution-semantics, Region-outcome, and callback facts;
- scheduler instance, exact registration, authored generation, event execution
  boundary, stable lifecycle boundary, one-shot/continuing-tick semantics, and
  running zero-run state must all agree, while the scheduler Store boundary
  must have been absent during Region work;
- a refused Region outcome is consistent only when it performed no mutation
  and did not satisfy desired state; its required action is to retain the exact
  event registration, running state, zero execution count, and unchanged
  lifecycle version;
- an applied outcome must report both mutation and desired-state satisfaction,
  while a no-op must report desired-state satisfaction without mutation; both
  require terminal consumption of the exact registration;
- terminal postconditions require registration removal, stopped running state,
  a newer lifecycle version, unchanged zero execution count, no callback
  invocation or reschedule, and Store acquisition only after Region work; and
- any identity, boundary, semantics, outcome, lifecycle, callback, registration,
  or ordering mismatch produces a typed refusal rather than a permissive result.

Automated validation status:

- an executable fixture proves refusal retains the exact event unchanged and
  both applied and no-op outcomes require and satisfy terminal consumption;
- the fixture also refuses a Store boundary held across Region work,
  inconsistent mutation/no-op claims, an already-invoked callback, retained
  events after desired-state satisfaction, consumed events after Region
  refusal, and invalid callback/execution post-state;
- source guards enforce the two required actions, inert authority flags, no
  runtime imports or mutation methods, and no runtime consumer;
- the complete layered-map suite passes 462 tests across 139 focused files;
  and
...
- this is a pure dormant contract. It does not stop an event, acquire a Store
  boundary, remove a registration, invoke a callback, or inspect/mutate a
  Region;
- callback non-invocation and no reschedule are mandatory for both retained and
  consumed outcomes;
- the contract distinguishes Region mutation from desired-state satisfaction,
  so an idempotent no-op still consumes the obsolete one-shot callback; and
- Store implementation, Region integration, arrival ordering, Region
  retirement, reconstruction, loading, persistence, and true layered storage
  remain later gates.
...
Objective: compose Slice 138/139's real Region object/collision commit with
Slice 141's exact scheduler/event consumption boundary while keeping the
combined operation unreachable from every production recovery, arrival, and
gameplay path.

Implemented:

- `GameTickEventStore.withValidatedRestorationRegionCommitConsumption` accepts
  the Region manager plus only expected scheduler identity, registration
  sequence, and authored proposal generation, then delegates every scheduler,
  event, lifecycle, semantics, and request check to Slice 141;
- the ephemeral request is passed directly to the existing
  `RegionManager.applyGameTickEventRestorationCommitRequest` seam while the
  event execution and stable lifecycle boundaries remain held and the Store
  monitor remains absent;
- the Region result maps by exact closed outcome name into Slice 140's
  disposition: `REFUSED` retains the exact event, while `APPLIED` and `NO_OP`
  remove and terminally consume it only after Region work returns;
- the composed result copies the Region outcome, typed reason, membership
  removal/registration facts, and ordered-boundary count alongside the closed
  scheduler result without retaining the Region result or a runtime handle;
  and
- a scheduler or lifecycle refusal occurs before Region invocation, so invalid
  registration/generation/event state cannot reach object or collision
  mutation.

Automated validation status:

- a deterministic composition fixture proves applied Region state consumes the
  exact registration, copies mutation/membership evidence, and never invokes
  callback code;
- the fixture proves an idempotent Region no-op also consumes the obsolete
  callback, while Region refusal retains the same running registration and
  lifecycle unchanged;
- stale authored generation proves the scheduler refusal path never calls the
  Region seam or constructs Region result evidence;
- Slice 138/139's real transaction fixtures continue proving exact empty spawn,
  exact removal, desired-state no-op, exact authored-transient replacement,
  rollback, and typed refusal inside the real ordered Region union;
- source guards require the existing real Region method, the Slice 141 consumer,
  closed authority flags, and no production caller;
- the complete layered-map suite passes 470 tests across 141 focused files;
  and
- the authoritative bundled-Ant server build compiles 792 core and 488 plugin
  sources and passes its build/classpath audit.
...
- the combined package-local method is executable and can mutate real Region
  membership/collision plus terminally consume one event, but no production
  caller reaches it;
- arrival and gameplay remain disconnected, as do parity diagnostics, Region
  loading/retirement, and reconstruction orchestration;
- no request, Region result, object, Region, event, Store, callback, monitor,
  commit token, arrival gate, or lifecycle authority escapes the operation;
  and
- selecting recovery candidates, ordering a batch, deciding overdue-versus-
  future callback behavior, invoking recovery before first visibility, and
  integrating loading/persistence remain separately gated.
...
Objective: turn Slice 158's safe but under-explained proposal refusal into one
bounded, deterministic event-recovery burden assessment before any scheduler
or Region recovery work can begin.

Implemented:

- live preparation first scans the already-bounded immutable event inventory
  in stable snapshot order and classifies every proposal-related callback as
  complete or by its first unmet recovery requirement;
- the closed requirement table distinguishes unavailable restoration state,
  missing atomic timing, stopped/already-executed state, incomplete detached
  payload/execution/target/arrival/generation/idempotency evidence, and authored
  generation mismatch;
- the preflight reports total proposal-related, recovery-complete, and
  recovery-incomplete counts, plus incomplete owner-position-hint and
  exact-spatial counts and the first stable incomplete registration,
  owner kind, attribution kind, and requirement;
- any incomplete proposal-related callback still refuses before live
  scheduler/Region capture. `recoveryCandidateCount` now means the number of
  complete recovery candidates rather than the number examined before an
  early refusal; and
- additive schema-v45 publishes the bounded preflight only on the existing
  explicit private `recover-noop` action. Schema-v44 remains immutable and
  ordinary trace events retain null recovery results.

Automated validation status:

- the executable preparation fixture mixes one NPC owner-position-hint
  callback without restoration state and one complete exact-spatial scenery
  callback, proves all counts and the first stable blocker, and proves no
  Region capture begins;
- the no-op diagnostic fixture proves both complete and incomplete preflight
  shapes while preserving zero mutation/consumption and the existing ready,
  overdue-refusal, and incomplete-refusal behavior;
- schema fixtures accept closed complete and blocked v45 preflights, reject
  inconsistent nullability/side-effect claims, and preserve v44 unchanged;
- source guards cover the current schema, stable requirement vocabulary,
  serializer fields, and AI-readable documentation;
- the complete layered-map suite passes 531 tests across 158 focused files;
  and
- the authoritative bundled-Ant server build compiles 804 core and 488 plugin
  sources and passes its build/classpath audit.
...
- the preflight scans only the detached bounded inventory and retains no
  scheduler, event, Mob, Region, object, or collision handle;
- it never discards an incomplete callback, shrinks the candidate/source
  envelope, or treats one complete callback as proposal readiness;
- no focused-subset execution, owner preservation, event cancellation,
  reschedule, reconstruction, loading, retry, arrival, visibility, teardown,
  or lifecycle authority is added; and
- whether NPC-owner callbacks may remain scheduled through explicit entity
  preservation requires a separate bounded correlation and policy.
...
- the corrected 36-source, generation-1 capture observed 457 proposal-related
  owner-position callbacks: 449 NPC-owned and 8 player-owned;
- all 449 NPC callbacks carried authored identity and each matched exactly one
  recognized active NPC with the expected definition and a current position
  inside its selected owner source;
- missing identity, stale generation, absent/duplicate active owner,
  unrecognized identity, definition mismatch, and position drift all remained
  zero;
- all 449 exact NPC matches correctly remained
  `OWNER_PRESERVATION_UNPROVED`, with zero continuity-eligible callbacks,
  because the runtime supplies no preservation fact; and
- the 8 non-NPC results were player-owned current-position hints in the
  player's selected source. They are a separate recovery class, not an NPC
  identity-correlation failure.

Status: implemented, automated-validated, and owner-validated. The evidence
closes active NPC identity correlation; actual owner preservation and
player-owned callback recovery remain unresolved.
...
- the corrected marker, recovery-noop, and stop records all populated the
  continuity block with identical outcome classes;
- event and census observations used the same tick and exact 36-source
  selection, and every position round trip remained exact;
- the recovery preflight still refused all 457 related callbacks before live
  work, with no reconstruction, recovery invocation, mutation, or terminal
  event consumption; and
- the result is therefore valid diagnostic evidence without weakening any
  recovery or lifecycle boundary.

Status: implemented, automated-validated, and owner-validated. The next slice
should use the now-closed NPC correlation result to define what a real,
...
- after those owners are known, every other inventory callback carrying the
  same exact authored NPC identity is added as a supporting event link even
  when it is not proposal-related. Related and supporting counts remain
  distinct, and the shared event-link budget covers both;
...
- an executable fixture proves two related callbacks and one non-related
  supporting callback deduplicate to one exact owner with all three stable
  registration links, while one player-owned callback remains separate;
...
Objective: close the transient NPC lifecycle portion of Slice 165's boundary
without holding an NPC monitor across arbitrary gameplay/event code or creating
a persistent global owner registry.

Implemented:

- Slice 164 requirements now first identify exact proposal-related owners, then
  scan the complete same-tick event inventory and retain every other callback
  carrying one of those exact authored NPC identities. Proposal-related and
  supporting callbacks remain separately counted, while one shared bounded
  registration list fences both;
- each NPC owns a small lifecycle gate. Normal movement, behavior, removal,
  respawn, teleport/location, and final Region-removal operations increment an
  in-flight count without retaining the gate monitor while their gameplay code
  runs;
- preservation refuses immediately when lifecycle work is already active,
  preventing the scheduler-event-to-NPC lock cycle that a blocking monitor
  acquisition could create. Once accepted, new lifecycle work waits until the
  short read-only preservation operation returns;
- the existing event/registration and World-list nesting enters each exact NPC
  preservation gate in stable owner order. At the innermost point it rechecks
  every event-owner object, authored identity, active state, and unique World
  membership while all gates remain active; and
- only that fully revalidated inner point reports the NPC lifecycle count and
  Region-absence quiescence as complete. A busy owner, changed registration,
  inactive owner, reference mismatch, or World mismatch refuses without
  waiting or publishing a partial preservation claim.

Locking and compatibility:

- the scheduler Store retains no coordinate, NPC, World, or `EntityList`
  dependency. The package-local adapter composes the Store's already-tested
  validated registration fence with the NPC and World gates;
- no NPC gate monitor is held while behavior, event scheduling/removal, plugin
  code, Region movement, or respawn code executes. This preserves the generic
  scheduler fixture boundary and avoids reversing the event-to-World-to-NPC
  order;
- World NPC registration/removal continues to share the existing synchronized
  `EntityList`; the inner observation holds that same list while owner gates
  are entered and references are revalidated; and
- the gate is reusable, refuses same-thread lifecycle reentry from inside its
  read-only callback, and resumes waiting normal work after release.

Safety boundary:

- this proves quiescence only while the nested operation is active. The
  detached result is stale after return and still reports false for a durable
  preservation fact, retained handle, performed preservation, registry,
  reschedule, arrival gate, and lifecycle authority;
- no persistent absent-Region state, freeze token, pin, lease, NPC relocation,
  source retirement, reconstruction, or recovery consumption is added;
- player-owned callbacks remain separate and are not absorbed into the NPC
  mechanism; and
- no callback is invoked, stopped, reset, removed, rescheduled, or recreated by
  the boundary, and no NPC, Region, collision, arrival, or visibility state is
  mutated by the observation.

Automated validation status:

- an executable concurrency fixture proves active lifecycle work refuses
  preservation, accepted preservation blocks later lifecycle entry until
  release, same-thread reentry fails, and the gate remains reusable;
- the existing requirement fixture proves one exact owner includes two related
  callbacks plus one non-related supporting callback while the player callback
  stays separate;
- source guards cover movement, behavior, remove/respawn, location, and final
  removal routes, inner correlation revalidation, quiescence publication, and
  continued scheduler Store isolation;
- the complete layered-map suite passes 552 tests across 165 focused files;
  and
- the authoritative bundled-Ant server build compiles 809 core and 488 plugin
  sources and passes its build/classpath audit.

Status: implemented and automated-validated. The next slice should expose this
scope-ready but non-durable result through a new additive private diagnostic
...
Objective: remove call-stack depth from Slice 167's event/NPC boundary while
preserving its exact all-or-nothing exclusion and fail-closed semantics.

Owner-test finding:

- schema-v47 records 1-3 were written normally for start, marker
  `owner-origin-3`, and the teleport from `(120,620)` to dense `(122,503)`;
- the later `owner-boundary` marker created the expected 36-source proposal and
  attempted to fence the same 449 NPC-owned callbacks seen in Slices 162-164;
- the server logged `StackOverflowError` in
  `captureNestedNpcLifecycleBoundaries`, with the same stack also retaining the
  recursively nested event-registration/timing frames from
  `captureNestedBoundaries`;
- the marker emitted no partial JSONL record and the server remained online.
  Other developer commands continued to work, while later parity captures
  retried the same oversized boundary and appeared inert; and
...
Objective: create the smallest runtime capability that can eventually contain
one source-absence/reconstruction operation without converting Slice 167's
detached observation into a reusable preservation proof.

Implemented:

- the NPC owner requirements now retain the event inventory's exact canonical
  packed-source coordinate order rather than only its source count;
- the runtime adapter exposes a package-local `withinPreservationScope`
  operation. It is invoked only after the exact event set, scheduler
  registrations, event timing, World NPC membership, owner identity and
  instance, NPC lifecycle set, and Region-absence quiescence all validate in
  one nested operation;
- the scope is invalidated before the enclosing gates release;
- the supplied scope binds exact generation, requirements observation tick,
  scheduler instance, selected packed sources, event-link count, and owner
  count without retaining an event, NPC, Region, scheduler registration, or
  other runtime handle;
- every scope accessor requires the original boundary thread and an active
  scope. The adapter invalidates the scope in `finally` before the enclosing
  NPC, World, registration, and event gates release;
- an incomplete or busy boundary never invokes the caller. A caller exception
  still invalidates the scope and then propagates; and
- ordinary schema-v47 capture continues to use the observation-only path. No
  production reconstruction, Region absence, recovery, retirement, or
  visibility consumer is connected.

Safety boundary:

- this is a transient admission capability, not a reusable preservation fact,
  lease, permit, commit token, entity registry, arrival gate, or lifecycle
  authority;
- selected sources are detached coordinate identities, not Region handles;
- no event or NPC callback, movement, removal, registration, reschedule, or
  state mutation is performed by opening or invalidating the scope; and
- the 8 player-owned callbacks remain separate and no proposal readiness is
  inferred from the NPC-only scope.

Automated validation status:

- an executable fixture proves exact scalar/source binding, immutable source
  order, boundary-thread confinement, post-invalidation refusal, double-close
  refusal, and rejection of every incomplete boundary dimension;
- the real detached requirement fixture proves the inventory's exact source is
  retained alongside the prior owner/event arithmetic;
- structural guards prove the scope opens only after inner owner revalidation,
  invokes the caller only inside that scope, invalidates in `finally`, and has
  no event, NPC, or Region dependency;
- the complete layered-map suite passes 563 tests across 168 focused files;
  and
- the authoritative bundled-Ant server build compiles 810 core and 488 plugin
  sources and passes its build/classpath audit.

Status: implemented and automated-validated. Owner validation is not required
because no production caller reaches the scope and schema-v47 behavior is
unchanged. The next slice should define a closed, disconnected source-lifecycle
completion result and prove that only a complete absence-and-reconstruction
...
- the exact pushed checkpoint emitted a four-record v48
  start/teleport/preservation-noop/stop segment, and all four records validate
  against the additive schema registry;
- the dense record at `(122,509,L0)` selected 36 packed sources and correlated
  457 proposal-related callbacks: 449 exact NPC owners plus 8 separate
  non-NPC/player callbacks;
- all 449 required owners and event links passed the exact registration,
  execution, timing, World-registration, NPC-lifecycle, reference, and
  Region-absence-quiescence boundaries, producing
  `PRESERVATION_SCOPE_READY`;
- the explicit no-op then reported `SOURCE_LIFECYCLE_UNAVAILABLE` with
  owner-scope entry and source-lifecycle invocation true, but zero absent
  sources, zero reconstructed sources, zero preserved-consumer invocations,
  and every mutation, handle, arrival, visibility, and lifecycle-authority
  field false; and
- the owner observed a fully loaded dense area and normal movement and
  interaction. Server/client evidence records successful NPC dialogue, gate
  operation, and scenery examination after the capture, with no command,
  logout, stack, or runtime failure.

Status: implemented, automated-validated, owner-validated, checkpointed, and
accepted. Slice 172 proves the real exact owner scope reaches the deliberately
...
Objective: inventory the exact runtime dependencies that still prevent a
selected packed-source set from becoming temporarily absent, without
performing source absence or granting lifecycle authority.

Implemented:

- `LayeredPackedRegionSourceAbsencePreflight` binds generation, requirements
  tick, capture tick, residency-mirror version, and exact selected-source order
  to immutable Region-local inventories;
- each source distinguishes tile-storage availability, active players, NPC
  membership, authored objects, identity-less dynamic objects, ground items,
  and collision-product tiles;
- stable typed blockers distinguish unavailable NPC membership preservation,
  authored reload, dynamic-object preservation, ground-item preservation,
  collision rebuild, and the Region reload path rather than collapsing them
  into one generic lifecycle refusal;
- source and whole-selection totals reconcile independently, and blocker
  summaries count affected exact sources rather than runtime instances; and
- RegionManager captures the inventory only while the Slice 173 boundary and
  `layeredRegionLifecycleLock` remain active, rechecks residency-mirror
  version and membership, uses non-creating Region lookup, and rejects
  unavailable collision storage.

Safety boundary:

- no Region becomes absent, unloaded, removed, created, reconstructed, or
  registered through the preflight;
- the result retains no Region, tile, entity, collection, registry, scheduler,
  event, or other runtime handle;
- Region storage, residency mirror, visibility caches, arrival, pathing,
  packets, persistence, and gameplay remain unchanged;
- every result is point-in-time evidence only and explicitly denies source
  mutation, reconstruction, arrival-gate, and lifecycle authority; and
- the capture remains disconnected from the parity observer and
  source-lifecycle consumer, so schema-v48 and the accepted private route are
  unchanged.

Automated validation status:

- an executable fixture proves exact source-order binding, detached survival
  after boundary invalidation, totals, stable blockers and summaries,
  immutable results, invalid inventory refusal, and every non-authoritative
  flag;
- structural guards prove RegionManager requires the real lifecycle monitor,
  mirror version and exact residency, performs only non-creating lookup and
  read-only retirement snapshots, and contains no removal, unload, registry,
  or cache mutation;
- the complete layered-map suite passes 583 tests across 173 focused files;
  and
- the authoritative bundled-Ant server build compiles 814 core and 488 plugin
  sources and passes its build/classpath audit.

Status: implemented and automated-validated. Owner validation is deferred
because the preflight has no private diagnostic caller yet. The next focused
slice should attach this detached result to the explicit
...
- the cooldown-aware `(120,620) -> (132,502)` route emitted four contiguous
  schema-v53 records, returned the diagnostic response normally, and produced
  no owner-observed visual, interaction, connection, or command issue;
- the entered preservation record aligns all 36 selected sources across
  reload, terrain, collision derivation, and collision application. Its 4,551
  replay placements, 3,979 object footprints/applications, 3,185 contribution
  references, and all inherited source identities and fingerprints reconcile;
- disposable work is exact: 72 pre-application Regions, 36 collision
  application Regions, 72 terrain applies, 36 object-membership applies, 3,979
  ordered collision applications/boundary acquisitions, and 82,944 verified
  Region tiles. The applied counters reconcile 2,989 blocking-scenery, 196
  dynamic-collision, and 2,554 dynamic-projectile contributions; and
- collision application occurred only on disposable Regions. Collision
  registration, runtime collision, retained handles, runtime-source mutation,
  membership/scheduler restoration, registry/residency/visibility mutation,
  arrival, visibility release, and lifecycle authority all remain false.
...
Objective: execute Slice 193's complete same-union proof for every exact
selected source under one bounded lifecycle observation, retaining only stable
count/fingerprint evidence.

Implemented:

- the accepted Slice 188 derivation batch gains one package-local composition
  path that retains its exact source bound and active definition capture, then
  invokes Slice 193 once per source. Its public capture remains
  derivation-only;
- one immutable batch aligns every combined receipt with the original source
  ordinal, coordinates, replay/object/contribution/Region counts, terrain and
  replay fingerprints, definition-capture fingerprint, and collision-footprint
  fingerprint;
- aggregate evidence separates the two pre-combination disposable Regions per
  source from each combined Region union, reports source and support Region
  counts, distinguishes terrain/membership/collision applications and boundary
  acquisitions, and reconciles verified tiles plus every collision family; and
- only the applied-collision and complete final-state fingerprints are added to
  the existing four stage fingerprints. Intermediate Region unions, objects,
  tiles, plans, definitions, boundaries, and runtime handles are discarded
  before the next source.

Safety boundary:

- source count uses the established refusal-based maximum of 128; the batch
  cannot truncate a larger selection into apparent completeness;
- the new batch is disconnected from diagnostics, loading, gameplay, packets,
  persistence, arrival, and production lifecycle;
- combined terrain, membership, and collision application occur only on
  locally constructed unregistered Region unions. Collision registration and
  every runtime-source, NPC/item, scheduler, active-family, registry,
  residency, visibility, arrival, and lifecycle-authority fact remain false;
  and
- Slice 188's public derivation-only capture, Slice 191's separate collision
  application batch, schema-v53, and accepted private diagnostics remain
  unchanged.

Automated validation status:

- the real source-lifecycle fixture executes the accepted derivation-only,
  separate collision-application, and new combined-state batches against the
  same deterministic source. Each retains five replay placements, three
  authored object footprints, four contribution references, four required
  Region references, and two unique collision Regions;
- the combined batch reconciles two pre-combination Regions with one
  two-Region final union, three combined object-membership applications and
  boundaries, three collision applications, four collision boundaries, and
  4,608 verified tiles. All six per-source fingerprints remain present and
  aligned;
- immutable result storage, the 128-source refusal bound, absence of
  disposable/runtime handles, and unchanged derivation-only public capture are
  source-guarded;
- the Slice 182-194 focused lineage passes 55 tests, including five Slice 194
  tests; and
- the authoritative bundled-Ant server build compiles 834 core and 488 plugin
  sources and passes its build/classpath audit.

Status: implemented and automated-validated. This disconnected batch does not
warrant an owner route. The next slice should expose only this bounded
count/fingerprint summary through the explicit private preservation action,
...
- four contiguous schema-v54 records cover start, quiet-to-dense teleport,
  `preservation-noop`, and stop, with the response returned normally;
- all 36 selected sources align across reload, authored-collision derivation,
  separate collision application, and combined state: 4,551 replay placements,
  3,979 object footprints/applications, 3,185 unique contribution tiles, 36
  combined Region unions, 108 total disposable Region constructions, and
  82,944 verified Region tiles;
- the combined collision totals reconcile exactly to 2,989 blocking-scenery,
  196 dynamic-collision, and 2,554 dynamic-projectile contributions, while all
  inherited, applied-collision, and final-state fingerprints are present and
  source-aligned; and
- the owner reported no missing response or issue, successfully interacted
  after the capture, and every runtime collision, mutation, retained-handle,
  registry, mirror, cache, arrival, visibility, and lifecycle-authority fact
  remains false.
...
Status: owner-validated. Four contiguous schema-v56 records validate and remain
round-trip exact. Across the selected 36-source set, all 3,979 expected authored
objects were observed as unique exact final-live instances with present,
constructor-matched registrations; all 3,185 contribution references and 3,979
Region references match the disposable transactional baseline. Every source
resolves to `EXACT_BASELINE_MATCH`, with zero transient, missing, duplicate,
unrecognized, identity-conflict, invalid-registration, or stable-mismatch
outcomes. The diagnostic response, stop response, and subsequent interaction
remained normal. One 1,127 ms late tick, including 995 ms of diagnostic work,
...
The accepted Slice 125 route proves that one real pending authored callback can
be rediscovered by scheduler scope and registration, matched to the proposal's
authored generation, classified under the Region object boundary, and checked
against a stable event lifecycle. It still proves only a point-in-time read: the
...
is a no-op, and stale or conflicting state refuses. Slice 139 now connects
Slice 128's exact transient snapshot to that same closed Region union: a zero-
attribute transient with symmetric unregister/rollback collision projection can
...
and Slice 141 executes it under the real exact registration, event execution,
and stable lifecycle boundaries. Slice 142 now composes that scheduler
consumer with the existing real Region object/collision commit: refusal retains
the pending callback, while applied/no-op desired state removes and terminally
stops it only after Region work. The closed result preserves the typed Region
...
Slice 159 now makes that preflight burden exact and bounded. Every
proposal-related callback is classified before live work; complete recovery
work, incomplete work, owner-position-hint/exact-spatial incomplete counts, and
the first stable blocker are distinct. One complete callback can no longer be
confused with either the number examined before refusal or proposal readiness.

The captured dominant blocker is not another scenery callback: it is an NPC
owner-position-hint event without standalone restoration state. The next
architecture step should determine, with bounded local correlation rather than
a global registry, which owner-bound callbacks can safely remain scheduled
because the exact NPC owner is explicitly preserved across a source lifecycle.
