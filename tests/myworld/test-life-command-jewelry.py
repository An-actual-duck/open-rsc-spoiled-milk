...
def main() -> None:
    ...
    for offset, tier in enumerate(TIERS):
        ...
        require(necklace["description"] == f"Extends support upkeep by {NECKLACE_DURATION[offset]}%; Life-rune upkeep grants +{NECKLACE_UPKEEP_XP[offset]}% Summoning XP.", f"incorrect Necklace description for {tier}")
    ...
    require("getSupportUpkeepDisplayedExperience(owner)" in consume, "only successful Life-rune consumption should award Necklace XP")
    upkeep_xp = method_body(summoning, "getSupportUpkeepDisplayedExperience")
    require("getLifeNecklaceSupportUpkeepXpBonusPercent()" in upkeep_xp, "support upkeep XP must use the Necklace")
