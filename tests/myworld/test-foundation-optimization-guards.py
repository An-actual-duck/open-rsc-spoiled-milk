...
def main() -> None:
    ...
    require_snippet(PATH_VALIDATION, "final Mob start,\n\t\tfinal Mob destination,", "layer-aware Mob adjacent-distance overload")
    ...
    require_snippet(MOB, "PathValidation.checkAdjacentDistance(\n\t\t\t\t\t\tMob.this, mob, true, false)", "Mob following layer-aware adjacent-distance check")
    require_snippet(PLAYER, "PathValidation.checkAdjacentDistance(\n\t\t\t\tthis, getFollowing(), true, false)", "Player following layer-aware adjacent-distance check")
    require_snippet(NPC_BEHAVIOR, "private boolean canEngageTargetFrom(final Point checkedPoint)", "NPC behavior scoped engagement helper")
    require_snippet(NPC_BEHAVIOR, "npc.getWorld(), checkedLocation, target.getWorldLocation(),", "NPC behavior authoritative adjacent-distance check")
    ...
    require_snippet(WALK_TO_MOB_ACTION, "boolean pathingCheckPassed = !sameTileCombatAttack\n\t\t\t&& checkAdjacentDistance(checkedPoint);", "walk-to-mob same-tile combat rejection")
    require_snippet(WALK_TO_MOB_ACTION, "checkedPoint.getX(),\n\t\t\t\tcheckedPoint.getY(),\n\t\t\t\tmob.getX(),\n\t\t\t\tmob.getY(),", "walk-to-mob primitive adjacent-distance check")
    ...
    require_snippet(NPC_COMMAND, "PathValidation.checkAdjacentDistance(\n\t\t\t\tplayer, affectedNpc, true, true)", "NPC command layer-aware adjacent-distance check")
    require_snippet(PVM_MELEE_EVENT, "boolean sameTile = attackerMob.sharesSpatialDomain(targetMob)", "PVM melee same-domain tile guard")
    require_snippet(PVM_MELEE_EVENT, "PathValidation.checkAdjacentDistance(\n\t\t\tattackerMob, targetMob, true, false)", "PVM melee layer-aware adjacent check")
    reject_snippet(PVM_MELEE_EVENT, "PathValidation.checkAdjacentDistance(attackerMob.getWorld(),", "PVM melee primitive adjacent-distance check")
