...
public final class ExtensionRegistryFixture {
 ...
 static final class DuplicateLease extends E { DuplicateLease(){super("duplicate-lease");} public void activate(ExtensionContext c){calls.add("+duplicate-lease");c.onDeactivate("same",()->calls.add("first-cleanup"));expect(IllegalArgumentException.class,()->c.onDeactivate("same",()->calls.add("replacement-cleanup")),"duplicate resource");} }
 ...
 public static void main(String[] a)throws Exception{
  ...
  calls.clear();r.discover(Collections.singletonList(new DuplicateLease()));r.activate(ExtensionContext.forTesting());r.deactivate();r.deactivate();check(calls.toString().equals("[+duplicate-lease, -duplicate-lease, first-cleanup]"),"duplicate registration preserves original cleanup exactly once");r.reset();
