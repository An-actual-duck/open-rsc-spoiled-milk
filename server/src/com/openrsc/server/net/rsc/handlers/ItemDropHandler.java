...
public final class ItemDropHandler implements PayloadProcessor<ItemCommandStruct, OpcodeIn> {
	...
	public void process(ItemCommandStruct payload, Player player) throws Exception {
		...
		int amount;
		boolean respectDropX = player.isUsingCustomClient()
			&& (player.getWorld().getServer().getConfig().WANT_DROP_X || player.getConfig().WANT_MYWORLD);
		if (respectDropX) {
			amount = payload.amount;
		} else {
			amount = 0;
		}
		final boolean dropAllMatching = player.getConfig().WANT_MYWORLD && amount < 0;
		if (dropAllMatching) {
