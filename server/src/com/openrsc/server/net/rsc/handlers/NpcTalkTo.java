...
public final class NpcTalkTo implements PayloadProcessor<TargetMobStruct, OpcodeIn> {
	...
	public void process(TargetMobStruct payload, Player player) throws Exception {
		...
		if (Summoning.isOwnedUtilitySummon(player, npc)) {
			Summoning.openUtilitySummon(player, npc);
			return;
		}
		...
		player.setWalkToAction(new WalkToMobAction(player, npc, 1) {
			public void executeInternal() {
				...
				if (npc.isBusy() || npc.getWorld().getServer().getGameClock()
						.currentTimeMillis() - npc.getCombatTimer()
						< player.getConfig().GAME_TICK * 5L) {
					if (npc.isBusy()
