			...
			switch (payload.getOpcode()) {
				...
				case SEND_PLAYER_COORDS:
					...
					for (Map.Entry<Integer, Integer> entry : mu.mobs) {
						if (entry instanceof MobsUpdateStruct.BitUpdate) {
							MobsUpdateStruct.BitUpdate bit = (MobsUpdateStruct.BitUpdate) entry;
							builder.writeBits(bit.getRawKey(), bit.getRawValue());
						} else {
