...
public class PlayerSettings {
	...
	public void toggleBlockDuel(Player player) {
		if (!player.getConfig().WANT_PVP) {
			setPrivacySetting(PRIVACY_BLOCK_DUEL_REQUESTS, (byte)BlockingMode.All.id());
			ActionSender.sendPrivacySettings(player);
			if (player.getConfig().WANT_MYWORLD) {
				player.playerServerMessage(MessageType.QUEST, "Duels are disabled on this PvM-only world");
			} else {
				player.playerServerMessage(MessageType.QUEST, "Duels are disabled on this world");
			}
			return;
		}
