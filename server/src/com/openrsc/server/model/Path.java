...
public class Path {
	...
	private final Deque<Point> waypoints = new ArrayDeque<>(MAXIMUM_SIZE);
	...
	public void addStep(int x, int y) {
		...
		while (Math.abs(diffX) > 0 || Math.abs(diffY) > 0) {
			...
			int lastX = last.getX();
			int lastY = last.getY();
			int nextX = x - (diffX - moveX);
			int nextY = y - (diffY - moveY);
			int xOnlyY = y - diffY;
			int yOnlyX = x - diffX;

			boolean canWalkX = PathValidation.checkAdjacent(mob, lastX, lastY, nextX, xOnlyY);
			boolean canWalkY = PathValidation.checkAdjacent(mob, lastX, lastY, yOnlyX, nextY);
			boolean canWalkXY = PathValidation.checkAdjacent(mob, lastX, lastY, nextX, nextY);
			...
			if (Math.abs(diffX) > 0 && Math.abs(diffY) > 0 && canWalkX && canWalkY) {
				...
				// Wall in the way, must zigzag.
					...
					boolean canWalkX2 = PathValidation.checkAdjacent(mob, nextX, xOnlyY, nextX, nextY);
					boolean canWalkY2 = PathValidation.checkAdjacent(mob, yOnlyX, nextY, nextX, nextY);
					if (canWalkX2)
						diffX -= moveX;
					else if (canWalkY2)
						diffY -= moveY;
