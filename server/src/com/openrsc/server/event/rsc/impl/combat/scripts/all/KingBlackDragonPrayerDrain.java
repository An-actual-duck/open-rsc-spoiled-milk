	...
	public void executeScript(Mob attacker, Mob defender) {
		if (attacker.isPlayer()) {
			if (attacker.getSkills().getLevel(Skill.PRAYER.id()) > 1)
				attacker.getSkills().setLevelFromStatReduction(Skill.PRAYER.id(), (int) Math.ceil((double) attacker.getSkills().getLevel(Skill.PRAYER.id()) * 0.04));

		} else if (defender.isPlayer()) {
			if (defender.getSkills().getLevel(Skill.PRAYER.id()) > 1)
				defender.getSkills().setLevelFromStatReduction(Skill.PRAYER.id(), (int) Math.ceil((double) defender.getSkills().getLevel(Skill.PRAYER.id()) * 0.04));
		}
