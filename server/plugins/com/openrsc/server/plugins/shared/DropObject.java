...
public class DropObject {
	public static void batchDrop(Player player, Item item, Boolean fromInventory, int amountToDrop, int totalToDrop, int invIndex) {
		...
		if (fromInventory && player.getConfig().WANT_MYWORLD && amountToDrop > 1) {
			int removingThisIteration = Math.min(amountToDrop,
				player.getCarriedItems().getInventory().countId(item.getCatalogId(), Optional.of(item.getNoted())));
			if (removingThisIteration <= 0 || item.getItemId() == -1) {
				player.message("You don't have the entered amount to drop");
				return;
			}

			long removedItemId = player.getCarriedItems().getInventory().remove(
				new Item(item.getCatalogId(), removingThisIteration, item.getNoted(), item.getItemId()),
				true,
				true);
			if (removedItemId == -1) {
				player.message("You don't have the entered amount to drop");
			...
			if (item.getNoted() || itemDef.isStackable()) {
				...
				for (int i = 0; i < removingThisIteration; i++) {
					GroundItem groundItem = new GroundItem(player.getWorld(), item.getCatalogId(), player.getX(), player.getY(), 1, player, false);
					player.getWorld().registerItem(groundItem, config().GAME_TICK * 300);
				}
			}
			player.getWorld().getServer().getGameLogger().addQuery(new GenericLog(player.getWorld(), player.getUsername() + " dropped " + item.getDef(player.getWorld()).getName() + " x"
				+ DataConversions.numberFormat(removingThisIteration) + " at " + player.getLocation().toString()));
			return;
		}
