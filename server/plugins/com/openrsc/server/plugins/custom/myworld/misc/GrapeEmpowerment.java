package com.openrsc.server.plugins.custom.myworld.misc;
...
public class GrapeEmpowerment implements UseInvTrigger {
	...
	public void onUseInv(Player player, Integer invIndex, Item item1, Item item2) {
		...
		if (compareItemsIds(item1, item2, ItemId.GRAPES.id(), ItemId.HOLY_SYMBOL_OF_SARADOMIN.id())) {
			...
			if (player.getLevel(Skill.HARVESTING.id()) < 70) {
				...
				delay(2);
				player.playerServerMessage(MessageType.QUEST, "You need a harvesting level of 70 to bless the grapes");
				return;
			}
			...
			if (currentPrayer < 1) {
				...
				delay(2);
				player.message("You need at least one available prayer point");
				return;
			}
			...
			if (player.getLevel(Skill.HARVESTING.id()) < 70) {
				...
				delay(2);
				player.playerServerMessage(MessageType.QUEST, "You need a harvesting level of 70 to curse the grapes");
				return;
			}
