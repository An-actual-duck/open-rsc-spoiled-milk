...
public class AgilityUtils {
	...
	public static void completedObstacle(Player player, int id, Set<Integer> obstacles, Integer lastObstacle, int bonus) {
		completedObstacle(player, id, obstacles, lastObstacle, bonus, ItemId.NOTHING.id());
	}

	public static void completedObstacle(Player player, int id, Set<Integer> obstacles, Integer lastObstacle, int bonus, int completionRewardId) {
		if (player.getAttribute("obstaclesDone") == null) {
			...
			if (obstacles.contains(id)) {
				...
				player.setAttribute("obstaclesDone", new HashSet<Integer>());
				if (completionRewardId != ItemId.NOTHING.id()) {
					Item pouch = new Item(completionRewardId, 1);
