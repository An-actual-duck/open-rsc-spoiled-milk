...
public class Tourist_Trap_Mechanism implements RemoveObjTrigger, UseNpcTrigger, OpLocTrigger, UseLocTrigger, UseInvTrigger, TakeObjTrigger,
	...
	public static double[] protoDartCraftRates;
	...
	public void onRemoveObj(Player player, Integer invIndex, UnequipRequest request) {
		...
		if ((item.getCatalogId() == ItemId.SLAVES_ROBE_BOTTOM.id() || item.getCatalogId() == ItemId.SLAVES_ROBE_TOP.id()) && (requestPlayer.getLocation().inTouristTrapCave()) && requestPlayer.getQuestStage(Quests.TOURIST_TRAP) != -1) {
			...
			if (n != null) {
				n.teleportCurrentScope(requestPlayer.getX(), requestPlayer.getY());
				requestPlayer.teleportCurrentScope(requestPlayer.getX(), requestPlayer.getY());
				delay();
				...
				requestPlayer.teleportCurrentScope(requestPlayer.getX(), requestPlayer.getY());
				Npc newNpc = addnpc(requestPlayer, NpcId.MERCENARY.id(), requestPlayer.getX(), requestPlayer.getY(), 30000);
				delay();
		...
		if (obj.getID() == 1006) {
			...
			if (menu == 0) {
				...
				if (protoDartSmithSuccessful(player.getSkills().getLevel(Skill.SMITHING.id()))) {
					...
					mes("You need to finish the prototype before it can be thrown.");
				} else {
	...
	private void attachFeathersToPrototype(Player player, Item i, Item i2) {
		if (compareItemsIds(i, i2, ItemId.FEATHER.id(), ItemId.PROTOTYPE_DART_TIP.id())) {
			...
			if (getCurrentLevel(player, Skill.CRAFTING.id()) < 10) {
				player.message("You need a crafting level of at least 10 to complete this.");
				return;
			}
			mes("You try to finish the bronze dart prototype.");
			delay(2);
			...
			if (protoDartCraftSuccessful(player.getSkills().getLevel(Skill.CRAFTING.id()))) {
				mes("You succesfully complete the prototype dart.");
				delay(2);
				...
				player.incExp(Skill.CRAFTING.id(), getMaxLevel(player, Skill.CRAFTING.id()) * 50, true);
			} else {
	...
	public static boolean protoDartCraftSuccessful(int craftingLevel) {
		if (protoDartCraftRates == null) {
			defineSuccessRates();
		}
		double successRate = protoDartCraftRates[craftingLevel];
		double roll = Math.random();
		return successRate > roll;
	}
	...
	private static void defineSuccessRates() {
		...
		protoDartCraftRates = new double[maxLevelToCalcFor];
		for (int level = 0; level < maxLevelToCalcFor; level++) {
			if (level >= 10) {
				protoDartCraftRates[level] = Formulae.interp(61, 254, level);
			}
		}
	}
	...
	private void driverCallGuards(Player player, Npc n) {
		...
		if (succeed == 0) {
			...
			if (mercenary != null) {
				mercenary = addnpc(player, NpcId.MERCENARY.id(), player.getX(), player.getY(), 60000);
				delay(2);
			}
	...
	private void failCaveAnaInBarrel(Player player, Npc n) {
		if (player.getCarriedItems().hasCatalogID(ItemId.ANA_IN_A_BARREL.id(), Optional.of(false))) {
			n = addnpc(player, NpcId.MERCENARY.id(), player.getX(), player.getY(), 60000);
			delay();
			...
			Npc ana = addnpc(player, NpcId.ANA.id(), player.getX(), player.getY(), 30000);
			delay();
	...
	public void onDropObj(Player player, Integer invIndex, Item item, Boolean fromInventory) {
		if (item.getCatalogId() == ItemId.ANA_IN_A_BARREL.id()) {
			...
			if (menu == 0) {
				...
				delay(3);
				player.getCarriedItems().remove(new Item(ItemId.ANA_IN_A_BARREL.id()));
				Npc Ana = addnpc(player, NpcId.ANA.id(), player.getX(), player.getY(), 20000);
				delay();
				...
				if (guard == null || guard.inCombat()) {
					guard = addnpc(player, NpcId.MERCENARY.id(), player.getX() + diffX, player.getY(), 30000);
				}
				delay();
