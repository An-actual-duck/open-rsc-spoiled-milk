...
public class DigsiteDigAreas implements OpLocTrigger, UseLocTrigger, OpInvTrigger {
	...
	private int[] DIGSITE_LEVEL3_ITEMS = {ItemId.NOTHING.id(), ItemId.BONES.id(), ItemId.DAMAGED_ARMOUR_1.id(), ItemId.BROKEN_STAFF.id(), ItemId.TALISMAN_OF_ZAROS.id(), ItemId.BROKEN_ARROW.id(), ItemId.BRONZE_SPEAR.id(), ItemId.PIE_DISH.id(), ItemId.BUTTONS.id(), ItemId.OLD_TOOTH.id(), ItemId.COINS.id(), ItemId.NEEDLE.id(), ItemId.CLAY.id(), ItemId.IRON_THROWING_KNIFE.id(), ItemId.LARGE_BLACK_HELMET.id(), ItemId.CERAMIC_REMAINS.id(), ItemId.BELT_BUCKLE.id(), ItemId.OLD_BOOT.id(), ItemId.PURPLEDYE.id()};
	...
	static void doDigsiteItemMessages(Player player, int item) {
		...
		} else if (item == ItemId.LARGE_BLACK_HELMET.id()) {
			player.message("You find a black helmet");
	...
	private void doSpade(Player player, Item item, GameObject obj) {
		...
		if (workmanCheck != null) {
			Npc workman = addnpc(player, NpcId.WORKMAN.id(), player.getX(), player.getY(), 30000);
