...
public class Chamber_Guardian extends AbstractShop {
	...
	public void onTalkNpc(Player player, Npc n) {
		if (player.getCache().hasKey("mage_arena")
			...
			(MageArena.getChosenGodCapeId(player) == ItemId.ZAMORAK_CAPE.id()
				|| MageArena.getChosenGodCapeId(player) == ItemId.SARADOMIN_CAPE.id()
				|| MageArena.getChosenGodCapeId(player) == ItemId.GUTHIX_CAPE.id()
			...
			npcsay(player, n, "good, good .. i hope you chose well",
				"you will have been rewarded with your god's blessing",
				"now i will give you your god staff",
				"these are all the weapons and armour you'll need here");
			player.message("the mage guardian gives you a god staff");
			if (MageArena.getChosenGodCapeId(player) == ItemId.ZAMORAK_CAPE.id()
				|| player.getCarriedItems().hasCatalogID(ItemId.ZAMORAK_CAPE.id(), Optional.of(false))) {
				give(player, ItemId.STAFF_OF_ZAMORAK.id(), 1);
			} else if (MageArena.getChosenGodCapeId(player) == ItemId.SARADOMIN_CAPE.id()
				|| player.getCarriedItems().hasCatalogID(ItemId.SARADOMIN_CAPE.id(), Optional.of(false))) {
				give(player, ItemId.STAFF_OF_SARADOMIN.id(), 1);
			} else if (MageArena.getChosenGodCapeId(player) == ItemId.GUTHIX_CAPE.id()
				|| player.getCarriedItems().hasCatalogID(ItemId.GUTHIX_CAPE.id(), Optional.of(false))) {
