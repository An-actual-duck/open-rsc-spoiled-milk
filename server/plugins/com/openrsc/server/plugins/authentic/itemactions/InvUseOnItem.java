...
public class InvUseOnItem implements UseInvTrigger {
	...
	private static final int[] DYE_IDS = {
		ItemId.REDDYE.id(),
		ItemId.YELLOWDYE.id(),
		ItemId.BLUEDYE.id(),
		ItemId.ORANGEDYE.id(),
	...
	private static final int[] CAPE_FAMILY = {
		ItemId.WOOL_CAPE.id(),
		ItemId.RED_CAPE.id(),
		ItemId.BLUE_CAPE.id(),
	...
	private static final int[] CAPE_DYE_OUTPUTS = {
		ItemId.RED_CAPE.id(),
		ItemId.YELLOW_CAPE.id(),
		ItemId.BLUE_CAPE.id(),
		ItemId.ORANGE_CAPE.id(),
	...
	public void onUseInv(Player player, Integer invIndex, Item item1, Item item2) {
		...
		if (compareItemsIds(item1, item2, ItemId.WOOL_WIG.id(), ItemId.YELLOWDYE.id())) {
			...
			if (player.getConfig().WANT_MYWORLD) {
				player.message("MyWorld fishing uses stronger fishing rods instead");
				return;
			}
		...
		int dyedCapeId = getDyedCapeOutput(item1.getCatalogId(), item2.getCatalogId());
		int capeId = item1.getCatalogId();
		int dyeId = item2.getCatalogId();
		if (dyedCapeId == -1) {
			dyedCapeId = getDyedCapeOutput(item2.getCatalogId(), item1.getCatalogId());
			capeId = item2.getCatalogId();
			dyeId = item1.getCatalogId();
		}
		if (dyedCapeId != -1) {
			if (capeId == dyedCapeId) {
				player.message("It is already dyed that color");
				return;
			}
			player.getCarriedItems().remove(new Item(capeId));
			player.getCarriedItems().remove(new Item(dyeId));
			player.message(getDyedCapeMessage(dyeId));
			player.getCarriedItems().getInventory().add(new Item(dyedCapeId));
	...
	private int getDyedCapeOutput(int capeId, int dyeId) {
		final int dyeIndex = ArrayUtils.indexOf(DYE_IDS, dyeId);
		if (dyeIndex < 0) {
			return -1;
		}
		if (ArrayUtils.contains(CAPE_FAMILY, capeId)) {
			return CAPE_DYE_OUTPUTS[dyeIndex];
		}
		return -1;
	}

	private String getDyedCapeMessage(int dyeId) {
		final int dyeIndex = ArrayUtils.indexOf(DYE_IDS, dyeId);
		final String color = DYE_NAMES[dyeIndex];
		return "You dye the cape " + color;
	}
	...
	public boolean blockUseInv(Player player, Integer invIndex, Item item1, Item item2) {
		...
		if(!player.getWorld().getServer().getConfig().CANT_DYE_CAPES) {
			...
			if (dyedCapeId != -1) {
				return player.getWorld().canYield(new Item(dyedCapeId));
			}
		}
