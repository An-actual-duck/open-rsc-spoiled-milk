...
/** Executable parity evidence for the bounded A07.5D slow proc. */
final class CurrentCombatEarthDragonSlowProcCharacterization {
	...
	private CurrentCombatEarthDragonSlowProcCharacterization() {
	}

	static void earthDragonSlowPolicies(
	...
	private static void assertSuccessfulZeroPrimaryDamageAndRefresh(
		...
		final Player source = harness.player(
			"Earth Success " + path.ordinal(), 890 + path.ordinal() * 8, 710);
		final Npc target = npcWithHits(
			harness, 891 + path.ordinal() * 8, 710, 20);
		equipEarthDragonSet(harness, source, 5);
		assertTrue(source.hasFullEarthDragonSet(),
			path + " full Earth Dragon set fixture");

		Object event = event(path, harness, source, target, 0);
		RandomExpectation random = primeSuccessfulRandom(
			harness, path, 7, 0xA075D1L);
		...
		random = primeSuccessfulRandom(harness, path, 0, 0xA075D2L);
		invokeLeatherEffects(path, event, source, target, 0);
	...
	private static void assertFailedRollConsumesOneDraw(
			...
			throws Exception {
		final Player source = harness.player(
			"Earth Failure " + path.ordinal(), 890 + path.ordinal() * 8, 720);
		final Npc target = npcWithHits(
			harness, 891 + path.ordinal() * 8, 720, 20);
		equipEarthDragonSet(harness, source, 5);
		...
		assertNoDamage(target, source, 20, 0,
			path + " failed earth roll");
		assertSlowState(target, 0, 0,
			path + " failed earth roll state");
		assertRandomTranscript(harness, path, random,
			path + " failed earth draw cardinality");
	}
	...
	private static void assertIneligibleCallsConsumeNoDraw(
		...
		final Player incompleteSource = harness.player(
			"Earth Partial " + path.ordinal(), 890 + path.ordinal() * 8, 730);
		final Npc incompleteTarget = npcWithHits(
			harness, 891 + path.ordinal() * 8, 730, 20);
		equipEarthDragonSet(harness, incompleteSource, 4);
		assertFalse(incompleteSource.hasFullEarthDragonSet(),
			path + " partial Earth Dragon set fixture");

		Object event = event(
			path, harness, incompleteSource, incompleteTarget, 7);
		RandomExpectation random = primeNoDrawRandom(harness, path, 0xA075D3L);
		invokeLeatherEffects(
			path, event, incompleteSource, incompleteTarget, 7);
		assertNoDamage(incompleteTarget, incompleteSource, 20, 0,
			path + " partial set earth damage");
		assertSlowState(incompleteTarget, 0, 0,
			path + " partial set earth slow");
		assertRandomTranscript(harness, path, random,
			path + " partial set draw cardinality");

		final Player deadTargetSource = harness.player(
			"Earth Dead " + path.ordinal(), 890 + path.ordinal() * 8, 740);
		final Npc deadTarget = npcWithHits(
			harness, 891 + path.ordinal() * 8, 740, 20);
		equipEarthDragonSet(harness, deadTargetSource, 5);
		deadTarget.getSkills().setLevel(Skill.HITS.id(), 0);

		event = event(path, harness, deadTargetSource, deadTarget, 7);
		random = primeNoDrawRandom(harness, path, 0xA075D4L);
		invokeLeatherEffects(path, event, deadTargetSource, deadTarget, 7);
		assertSlowState(deadTarget, 0, 0,
			path + " dead target earth slow");
		assertRandomTranscript(harness, path, random,
			path + " dead target draw cardinality");

		final Npc npcSource = harness.npc(
			NpcId.CHICKEN.id(), 890 + path.ordinal() * 8, 750);
		final Npc npcTarget = npcWithHits(
			harness, 891 + path.ordinal() * 8, 750, 20);
		event = event(path, harness, npcSource, npcTarget, 7);
		random = primeNoDrawRandom(harness, path, 0xA075D5L);
		invokeLeatherEffects(path, event, npcSource, npcTarget, 7);
		assertSlowState(npcTarget, 0, 0,
			path + " NPC source earth slow");
		assertRandomTranscript(harness, path, random,
	...
	private static Object event(final ProcPath path,
		...
		switch (path) {
			...
			default:
				throw new AssertionError("Unhandled earth path " + path);
		}
	}
	...
	private static void equipEarthDragonSet(
	...
	private static RandomExpectation primeFailedRandom(
			final CurrentCombatHarness harness, final ProcPath path) {
		if (path == ProcPath.PVM_MELEE) {
			harness.random().reset(0xA075D6L);
	...
	private static long findSuccessfulLegacySeed(final int rolledDamage) {
		...
		throw new AssertionError("Unable to find successful earth seed");
	}
	...
	private static long findFailedLegacySeed() {
		...
		throw new AssertionError("Unable to find failed earth seed");
	}
